### Figure S2 ###
# Content: Alpha Diversity
library(here)
library(tidyr)
library(dplyr)
library(ggplot2)
library(cowplot)
library(nlme)
library(ggpubr)

## Environment --

here::i_am("Rproj_slccolonpaper/Archived_Scripts/Trios_Alpha_Diversity.R")

### Function for plotting alpha diversity ---
generate_adiv_plots <- function(input_data, X, Y, min, max){
  #read in files
  data <- as.data.frame(input_data)
  
  #declare order of variables
  data$Genotype <- factor(data$Genotype, levels=c("WT", "HET","MUT"))
  #graph plot
  ggplot(data=data,aes(x={{X}},y={{Y}}, fill=Genotype)) + 
    geom_boxplot(alpha=0.25)+
    scale_fill_viridis_d()+
    #geom_line(aes(group = MouseID,color=Genotype),size=1)+
    geom_point(size=2,position=position_jitter(width=0.25),alpha=1)+
    theme_cowplot(16) +
    ylim(min,max) +
    theme(legend.position = "none")
  
}

## Colon lumen --
otus<- readr::read_delim(here("Trios/alpha_diversity/alpha_min_10000_Luminal_Colon_notax_Trios_ASV_table_Silva_v138_1/otus_dir/alpha-diversity.tsv"))
row.names(otus) <- otus$...1
shannon<-readr::read_delim(here("Trios/alpha_diversity/alpha_min_10000_Luminal_Colon_notax_Trios_ASV_table_Silva_v138_1/shannon_dir/alpha-diversity.tsv"))
row.names(shannon) <- shannon$...1
chao1<-readr::read_delim(here("Trios/alpha_diversity/alpha_min_10000_Luminal_Colon_notax_Trios_ASV_table_Silva_v138_1/chao1_dir/alpha-diversity.tsv"))
row.names(chao1) <- chao1$...1

data<- merge(otus,shannon, by="...1")
data<- merge(data,chao1, by="...1")
data$SampleID <- data$...1

metadata<- readr::read_delim(here("Trios/starting_files/SLC_TOTAL_OCT2020_FULL_Metadata.tsv"))
t_data_meta <- merge(data,metadata, by="SampleID")
t_data_meta$Subset <- dplyr::recode(t_data_meta$Subset,Luminal_Colon = "Colon Lumen")

## Colon mucosa --
otus<- readr::read_delim(here("Trios/alpha_diversity/alpha_Mucosal_Colon_notax_Trios_ASV_table_Silva_v138_1/otus_dir/alpha-diversity.tsv"))
row.names(otus) <- otus$...1
shannon<-readr::read_delim(here("Trios/alpha_diversity/alpha_Mucosal_Colon_notax_Trios_ASV_table_Silva_v138_1/shannon_dir/alpha-diversity.tsv"))
row.names(shannon) <- shannon$...1
chao1<-readr::read_delim(here("Trios/alpha_diversity/alpha_Mucosal_Colon_notax_Trios_ASV_table_Silva_v138_1/chao1_dir/alpha-diversity.tsv"))
row.names(chao1) <- chao1$...1

data<- merge(otus,shannon, by="...1")
data<- merge(data,chao1, by="...1")
data$SampleID <- data$...1

metadata<- readr::read_delim(here("Trios/starting_files/SLC_TOTAL_OCT2020_FULL_Metadata.tsv"))
t_mucosal_data_meta <- merge(data,metadata, by="SampleID")
t_mucosal_data_meta$Subset <- dplyr::recode(t_mucosal_data_meta$Subset,Mucosal_Colon = "Colon Mucosa")

### Make and store plots ---
adiv_trios_shannon<- generate_adiv_plots(t_data_meta, Genotype, shannon_entropy, 0, 8) +
  facet_wrap(Sex~Subset)+
  labs(x="",y="shannon")+
  ggtitle("2 month-old")+
  theme(plot.title = element_text(hjust = 0.5))
adiv_trios_shannon

adiv_trios_otus<- generate_adiv_plots(t_data_meta, Genotype, observed_features, 0, 600) +
  facet_wrap(Sex~Subset)+
  labs(x="",y="# ASVs")+
  ggtitle("2 month-old")+
  theme(plot.title = element_text(hjust = 0.5))
adiv_trios_otus

mc_adiv_trios_shannon<- generate_adiv_plots(t_mucosal_data_meta, Genotype, shannon_entropy, 0, 8) +
  facet_wrap(Sex~Subset)+
  labs(x="",y="shannon")+
  ggtitle("2 month-old")+
  theme(plot.title = element_text(hjust = 0.5))
mc_adiv_trios_shannon

mc_adiv_trios_otus<- generate_adiv_plots(t_mucosal_data_meta, Genotype, observed_features, 0, 600) +
  facet_wrap(Sex~Subset)+
  labs(x="",y="# ASVs")+
  ggtitle("2 month-old")+
  theme(plot.title = element_text(hjust = 0.5))
mc_adiv_trios_otus


### Baseline ---
otus <- readr::read_delim(here("Baseline/alpha_diversity/alpha_min_10000_Baseline_ASV_table_Silva_v138_1/otus_dir/alpha-diversity.tsv"))
row.names(otus) <- otus$...1
shannon<-readr::read_delim(here("Baseline/alpha_diversity/alpha_min_10000_Baseline_ASV_table_Silva_v138_1/shannon_dir/alpha-diversity.tsv"))
row.names(shannon) <- shannon$...1
chao1<-readr::read_delim(here("Baseline/alpha_diversity/alpha_min_10000_Baseline_ASV_table_Silva_v138_1/chao1_dir/alpha-diversity.tsv"))
row.names(chao1) <- chao1$...1

data<- merge(otus,shannon, by="...1")
data<- merge(data,chao1, by="...1")
data$SampleID <- data$...1

metadata<- readr::read_csv(here("Baseline/starting_files/Baseline_Metadata.csv"))
metadata$SampleID <- metadata$...1
data_meta <- merge(data,metadata, by="SampleID")
data_meta <- data_meta %>% filter(Line!="Q22")

### Make and store plots ---

baseline_shannon<- generate_adiv_plots(data_meta, Genotype, shannon, 0, 8) +
  ggtitle("3-4 month-old")+
  facet_wrap(Sex~Subset)+
  xlab("")+
  theme(plot.title = element_text(hjust = 0.5))
baseline_shannon

baseline_otus<- generate_adiv_plots(data_meta, Genotype, observed_otus, 0, 600) +
  ylab("# ASVs") +
  xlab("")+
  facet_wrap(Sex~Subset)+
  ggtitle("3-4 month-old")+
  theme(plot.title = element_text(hjust = 0.5))
baseline_otus

### Final Figure ---
dev.new(width=10,height=10)
plot_grid(adiv_trios_shannon,adiv_trios_otus,
          baseline_shannon, baseline_otus,
          nrow=2, labels=c("A","B", "C","D"))

dev.new(width=10,height=10)
plot_grid(mc_adiv_trios_shannon,mc_adiv_trios_otus,
          nrow=1, labels=c("E","F"))

### Statistics ---

## Trios --
f_t_data_meta <- t_data_meta %>% filter(Sex=="Female")
f_t_data_meta$Genotype <-factor(f_t_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lme(fixed= shannon_entropy ~ Litter + Site+ Genotype, random = ~1|MouseID, data=f_t_data_meta)
summary(output)

f_t_data_meta <- t_data_meta %>% filter(Sex=="Female")
f_t_data_meta$Genotype <-factor(f_t_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lme(fixed= observed_features ~ Litter + Site+ Genotype, random = ~1|MouseID, data=f_t_data_meta)
summary(output)

m_t_data_meta <- t_data_meta %>% filter(Sex=="Male")
m_t_data_meta$Genotype <-factor(m_t_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lme(fixed= shannon_entropy ~ Litter + Site+ Genotype, random = ~1|MouseID, data=m_t_data_meta)
summary(output)

m_t_data_meta <- t_data_meta %>% filter(Sex=="Male")
m_t_data_meta$Genotype <-factor(m_t_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lme(fixed= observed_features ~ Litter + Site+ Genotype, random = ~1|MouseID, data=m_t_data_meta)
summary(output)


f_t_mucosal_data_meta <- t_mucosal_data_meta %>% filter(Sex=="Female")
f_t_mucosal_data_meta$Genotype <-factor(f_t_mucosal_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lme(fixed= shannon_entropy ~ Litter + Site+ Genotype, random = ~1|MouseID, data=f_t_mucosal_data_meta)
summary(output)

f_t_mucosal_data_meta <- t_mucosal_data_meta %>% filter(Sex=="Female")
f_t_mucosal_data_meta$Genotype <-factor(f_t_mucosal_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lme(fixed= observed_features ~ Litter + Site+ Genotype, random = ~1|MouseID, data=f_t_mucosal_data_meta)
summary(output)


m_t_mucosal_data_meta <- t_mucosal_data_meta %>% filter(Sex=="Male")
m_t_mucosal_data_meta$Genotype <-factor(m_t_mucosal_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lme(fixed= shannon_entropy ~ Litter + Site+ Genotype, random = ~1|MouseID, data=m_t_mucosal_data_meta)
summary(output)

m_t_mucosal_data_meta <- t_mucosal_data_meta %>% filter(Sex=="Male")
m_t_mucosal_data_meta$Genotype <-factor(m_t_mucosal_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lme(fixed= observed_features ~ Litter + Site+ Genotype, random = ~1|MouseID, data=m_t_mucosal_data_meta)
summary(output)

## Baseline --

f_data_meta <- data_meta %>% filter(Sex=="Female")
f_data_meta$Genotype <-factor(f_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lm(shannon ~ Genotype, data=f_data_meta)
summary(output)

f_data_meta <- data_meta %>% filter(Sex=="Female")
f_data_meta$Genotype <-factor(f_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lm(observed_otus ~ Genotype, data=f_data_meta)
summary(output)

m_data_meta <- data_meta %>% filter(Sex=="Male")
m_data_meta$Genotype <-factor(m_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lm(shannon ~ Genotype, data=m_data_meta)
summary(output)

m_data_meta <- data_meta %>% filter(Sex=="Male")
m_data_meta$Genotype <-factor(m_data_meta$Genotype, levels=c("WT", "HET","MUT"))
output <- lm(observed_otus ~ Genotype, data=m_data_meta)
summary(output)

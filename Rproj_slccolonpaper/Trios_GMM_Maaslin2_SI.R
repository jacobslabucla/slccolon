...
pathway <- read.delim("Trios/Trios_OMIXER/GMM/modules.tsv", header = TRUE, row.names=1)
...
fit_data = Maaslin2(input_data=df_input_data, 
                    ...
                    output = paste0("Trios/Trios_OMIXER/GMM/PICRUST2_PWY_Luminal_SI_Maaslin2_SequencingRun_Site_Sex_Genotype_1-MouseID"), 
...
fit_data = Maaslin2(input_data=df_input_data, 
                    input_metadata=df_input_metadata, 
                    output = paste0("Trios/Trios_OMIXER/GMM/PICRUST2_PWY_Mucosal_SI_Maaslin2_SequencingRun_Site_Sex_Genotype_1-MouseID"), 
                    fixed_effects = c("Sequencing_Run","Site",  "Sex", "Genotype"), normalization = "TSS", 
                    random_effects = c("MouseID"),
                    #reference = c("Genotype,WT", "Site,Distal_Colon"),
                    min_prevalence = 0.15,
...
data<-read.table("Trios/Trios_OMIXER/GMM/PICRUST2_PWY_Mucosal_SI_Maaslin2_SequencingRun_Site_Sex_Genotype_1-MouseID/significant_results.tsv", header=TRUE)
annotation <- read.csv("Revised_Module_Key.csv")
data <- merge(data,annotation, by="feature")
data$description <- data$annotation
...
data<-read.table("Trios/Trios_OMIXER/GMM/PICRUST2_PWY_Luminal_SI_Maaslin2_SequencingRun_Site_Sex_Genotype_1-MouseID/significant_results.tsv", header=TRUE)
annotation <- read.csv("Revised_Module_Key.csv")
data <- merge(data,annotation, by="feature")
data$description <- data$annotation
...
res_plot <- data %>%
  ...
  unique() %>%
  mutate(site = ifelse(coef < 0, "WT", "MUT"))
pwy_mut <- create_bar_plot(res_plot, "Luminal SI: WT vs MUT q<0.25")
res_plot <- data %>%
  filter(value == "HET") %>%
  unique() %>%
  mutate(site = ifelse(coef < 0, "WT", "HET"))
pwy_het <- create_bar_plot(res_plot, "Luminal SI: WT vs HET q<0.25")
plot_grid(pwy_mut, pwy_het, labels = c("A", "B"))
